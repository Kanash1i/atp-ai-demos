...
### ⚠️ 别把这两条讲成「一条解决不了、要靠另一条补」—— 它们是不同层级

我一开始就是这么写的，**而且方向搞反了**。正确的分法：

| | 目的 | 能不能做到完备 |
|---|---|---|
| **补工具**（第 2 条） | 降低**失灵频率** | ❌ **永远做不完** —— 你枚举不完 agent 想做的动作 |
| **凭证边界**（第 1 条） | 限制**爆炸半径** | ✅ **能** —— 它是二元的，拿不到就是拿不到 |

**「agent 缺工具就自己凑」是失灵行为，它一定会发生，消除不了。**
所以真正要保证的不是「让它别失灵」，而是 —— 用户的原话 ——

> **「就算失灵了也不能直接写到数据库。」**

**凭证边界不是「不够」，它恰恰是唯一一道不依赖 agent 表现良好的防线。**
补工具让失灵变少，但你永远不能拿"少"当安全保证；
凭证边界让失灵的后果被框住，**而这个保证不随模型能力、提示词质量、agent 心情变化。**

> **用「让 agent 少犯错」替代「犯错了也不能出事」，是安全设计里最不该犯的错。**
> 前者是质量，后者是安全 —— 质量措施做到 99% 仍然是 99%，
> 而安全措施的意义正在于覆盖剩下那 1%。

### 迁移之后，爆炸半径具体被框成什么样

agent 手上只有一个 scope 到「创建/提交案例」的 token。即使它完全失灵、绕过 CLI 直接打平台 REST：

- ✅ **做得到**：写出一条烂案例 —— 但那和它通过 CLI 能做的事一样，没有额外风险
- ❌ **做不到**：`DROP TABLE`、删别人的案例、读别的项目的数据、改执行记录

**"写出烂案例"是可接受的失灵；"删库"不是。凭证边界正是把后者切掉的那一刀。**

至于「编一个假的成功回复骗用户」—— 那是另一类失灵（误导人，不损坏数据），
严重性和"直接写库"差一个量级，不该混在一起谈。
...
## D-127 · 新增 `atp inspect` —— 用工具消灭编造，而不是用约束

**由平台侧发起**（它给了完整契约与已验证的接口），我这边实现 CLI 命令。

### 为什么加它

D-124 记的那个失败已复现两次：agent 把商品详情页 URL 编成 `/product/p001`，
真实路由是 `/products/{id}`。**两次都通过了 `validate` 与 STD 规范校验**，
然后 404、等待超时、执行失败。

平台侧原计划在 prompt 里加约束（"不要编造 URL，不确定就问用户"），
按 D-123 那条观察改成了**先给它一个能真的看一眼的工具**：

> **约束只会让它换个方式绕，工具才是真的把路堵上。**

### 为什么放在 CLI 而不是只给平台 agent

用户定的方向：**CLI 是两个 agent 唯一的工具层** ——
平台 agent 也不直接调平台自己的接口，一律经 CLI。

**加一个工具只改 CLI 一处，两个 agent 同时获得。**
这跟「两个 agent 共用同一条写入路径」是同一条思路的延伸：
**共用的不只是写入格式，是整个工具层。**

> ⚠️ 顺带纠正一个我和平台侧都错过的判断：曾以为「opencode 离被测系统更近，
> 补这个能力成本更低」。**客户是测试人员，没有源码也不看代码**，
> 两边到被测系统等距、面对同一个问题 —— 这正是这个工具该放在 CLI 的理由。

### 实现上的三个决定

**① CLI 原样透传路径，不做任何解析。**
三种写法（相对路径 / 完整 URL / `${base_url}/…`）全部直接塞进请求体。
**解析规则只应存在于一个地方** —— 放两处迟早不一致。
由 `TestInspect_PassesPathThroughVerbatim` 锁定。

**② 12 与 20 严格分开**（同 D-116 的判据）：

| 情况 | 码 | 语义 |
|---|---|---|
| 404 | **12** | **你查错了** —— 换路径或问用户，原因进 `violations` |
| 503 / 连不上 / 返回非 JSON | **20** | **环境坏了** —— 重试或如实报告，别改案例 |

**都返回"探查失败"的话，agent 分不清是自己错了还是环境错了，大概率退回编造** ——
而编造正是这个工具要消灭的东西。

**③ 它是第一个不碰数据库的命令。**
只要 `ATP_API_URL`，一个 DB 凭证都不要。
...
1. **`locatorValue` 是规范允许的写法**（优先 `data-testid`，绝不产出绝对路径 XPath，
   带随机后缀的 id 跳过）。**agent 照抄就能过校验 —— 这是这个工具有用的前提。**
2. **探查在执行机上跑。** 案例最终在执行机资源池里跑，
   **探查环境必须与执行环境是同一个** —— 否则看到的 DOM 未必是执行时看到的 DOM，
   探出来的定位器仍可能跑不通。**这条是这个设计的地基。**

### 测试

`internal/cli/inspect_test.go` 用 `httptest` 桩掉平台，**测试不依赖平台在跑**：
200 正常路径 / 三种写法原样透传 / 404→12 且带 violations /
503→20 且不带 violations / 连不上→20 / 无 DB 凭证也能跑。

---

## D-128 · 加 `atp run`，但**刻意不做自动重试闭环**

由平台侧发起（给了完整契约与三份实测响应体），CLI 侧实现。

用途：agent 提交案例后跑一次自验。**只跑一次、如实报告、人决定改不改。**

### 为什么不做「失败就改、改完再跑」

平台侧给的两个理由，任何一个都足以否掉自动重试：

**① 执行失败 ≠ 案例写错了。**
被测系统真有 bug 时，自动改案例会把 bug 改没 —— **而发现 bug 正是测试的目的。**
一个会自动绕开失败的测试系统，等于没有测试系统。

**② 改到能跑通 ≠ 改对了。**
以"跑通"为目标，agent 最省力的路径是**削弱断言**：断言不了就删掉，等不到就放宽。
**测试变绿，但什么也不保证了。**

> 这是「给 agent 定错优化目标」的教科书案例：
> 目标写成"让它绿"，它就会去优化"绿"，而不是优化"对"。

### ⭐ 退出码映射和 `inspect` 不同 —— 这里最容易照搬错

| | 200 | 失败态 |
|---|---|---|
| `inspect` | 0 | **404 → 12**（你路径写错了，agent 自己能改）|
| `run` | 0（**含案例 FAILED**）| **504 → 20**（拿不到结论，环境问题）|

**案例 FAILED 用 0，不用 12。** 因为 FAILED 未必是 agent 的问题 ——
用 12 会诱导它去「修正」一份本来就对的案例，正好落进上面第 ① 条的坑里。

**这两条命令放在一起看，正好说明退出码的分派维度是「谁该负责」而不是「成没成功」：**
- 404：**你**（agent）写错了 → 12
- FAILED：**不知道是谁**的问题，但结论拿到了 → 0，交给人判断
- 504：**环境**坏了 → 20

### 一个不变量：`terminal` 与 `status` 不能合并

`terminal` 是「有没有拿到结论」，`status` 是「结论是什么」。
合并之后 agent 分不清「案例有问题」和「环境有问题」，
会把"没有执行机在线"误当成自己写的案例不行，**然后开始改一份本来没问题的案例。**

又是 D-116 那条判据：**不能让模型从错误信息里推断不出下一步动作。**

### 实现细节

- `--timeout` 原样透传，CLI 不裁剪上限（规则只放平台一处）
- **HTTP 客户端超时 = `timeoutSec` + 30s** —— 必须比平台等得久，
  否则平台还在等执行机、CLI 先超时，agent 拿到 20 而不是真实结论
- 人类可读模式只显示 `errorMsg` 第一行（Playwright 堆栈很长，人看不下去），
  FAILED 时附一句「跑挂了不等于案例写错了，别削弱断言让它变绿」
- 第二个不碰数据库的命令，只要 `ATP_API_URL`

### 测试

`internal/cli/run_test.go`，桩用的是**平台侧实跑出来的三份响应体原文**：
PASSED→0 / FAILED→0（且 `errorMsg` 的换行不被弄坏）/ 504→20 且提示"别改案例" /
`timeout` 原样透传 / 人类输出含防误导提示且不打整坨堆栈 / 无 DB 凭证也能跑。


---

## D-129 迁移是计划内功能；押后的只是它的**顺序**，不是它本身

**2026-08-31。** 我一度把「CLI 改调平台 API」当成可选项来权衡，**这是理解错了**。
用户明确：**改调 API 必须做，是计划内的功能实现。CLI 持有数据库账号密码
本来就是迭代的中间过程**，不是一个需要论证要不要离开的稳态。

记这条是因为我差点把它写成「暂不迁移」的决策 —— 那会让后来的 session 以为
直连 PG 是个可以长期停留的选择。**它不是。**

### 但有一件事仍然成立，只是位置变了

`internal/store` 下有 **19 个测试函数，并发 / 幂等 / TOCTOU 全在里面** ——
10 个 goroutine 真并行打同一个 key、CAS 落空的三分支诊断、唯一约束回放。

迁移之后，幂等和 CAS 的实现挪到平台侧，`CaseStore` 退化成 HTTP 客户端，
这 19 个测试不再验数据库行为，只能对着桩验「响应码 → exit code」——
**而那个映射是纯函数，没什么好并发测的。**

查证过平台侧（`grep -rln "CountDownLatch|ExecutorService|Thread(" */src/test` → 空）：
**并发与幂等的实现在**（`CaseWriteMapper` 全手写 CAS SQL、审批的并发仲裁），
**但目前没有一行测试压过它们。**

所以这不是「要不要迁」的理由，是**迁移里必须一并安排的一项工作**：

```
① 平台侧补上对着真 PG 的并发/幂等测试   ← 不做这步，能力就是净减少
② 迁写路径
③ 这边 19 个换成映射测试
```

漏掉 ①，迁移的净效果是**把整个 demo 里唯一能拿数字说话的部分弄没**。
硬纪律「评估先于功能」管这种情况和管「功能做完评估没跑」一样多 ——
demo1 是后者，这里差点是前者。

### 押后的是另外两件（它们才真的是可选）

**`atp lint-locator`**：价值已被 `inspect_page` 吃掉一大半 —— 探查返回的定位器
本来就按 STD 规则筛过（优先 `data-testid`、跳过动态 id、绝不出绝对路径），
agent 照抄就合规。剩下的用途是给**人手写的存量案例**做体检，是另一件事。

**M5 草稿清理任务**：演示不依赖。

### 顺带

平台侧 PR #38 补的三处字段**跟迁移无关，是现在就存在的真 bug**：
前端调 `POST /api/cases/draft` 建 iOS 案例会被静默变成 PC_WEB，
`DraftView.status` 返回 `tc_case.status` 而非 `tc_step.status`。M4 前端接线必然撞上。

---

## D-130 迁移完成：对拍全绿之后才删旧实现

**2026-09-01。** CLI 从直连 PostgreSQL 改为调平台 API，四步走完。
这条记的是**删除的依据** —— 因为被删掉的东西本身就是依据，不写下来就没了。

### 删之前，对拍 9 条脚本全绿

同一组输入分别喂 pgx 实现与 HTTP 实现，比 `model.Result`：

```
✓ 正常路径 draft → update → commit      ✓ draft 重试同一个 uuid
✓ update 响应丢失后重试（同内容）        ✓ 拿过期 version 提交（TOCTOU）
✓ update 同 version 不同内容（是冲突）   ✓ 提交后再改（状态冲突）
✓ commit 响应丢失后重试                  ✓ 不存在的案例
                                        ✓ 内容不合规范（校验失败）
```

**9 条里 6 条是失败/重放路径。** 这不是凑数：只跑正常流程的对拍，
漏掉的恰好是最容易分叉的分支 —— 实际分叉过的那一条
（`update` 的重放判据）就在失败路径上。

### 顺序纪律：证据先于删除

> 先用对拍验一次全绿，**再**删掉它和被它验证的旧实现。
> 反过来就是在没有证据的情况下拆掉证据。

这跟 D-129 那条「迁移的前置是平台侧先有等价证据」是同一条，换到了删除侧。
**证据和被验证物一起消失，是最容易发生的那种退化** —— 因为删的时候
两个都"没用了"，看起来是同一批垃圾。

### 那 19 个并发/幂等测试去哪了

删了。它们验的是 pgx 实现的 CAS 与幂等，而那个实现不存在了。
**等价能力在平台侧**：`CaseWriteConcurrencyTest`（4 个用例，对着真 PG，
10 线程并发 draft / update CAS / 串行推进 / 审批并发决策）。
这是 D-129 里给迁移定的前置条件 ①，平台侧在 PR #41 落地。

在 git 历史里可回溯（`git log -- internal/store/`）。

### 迁移真正关掉的两个洞

**① 凭证边界。** CLI 只持 `ATP_CLIENT_ID` / `ATP_CLIENT_SECRET`，
数据库账号密码只在平台那一侧。不是靠约束 agent 别碰库，是它根本拿不到。
（D-123 记的那次事故 —— agent 找不到删草稿的工具，就读 `.env` 拼 SQL
删了 —— 在这个架构下不可能发生。）

**② 规则强制。** 这条是做对拍时才发现的：**CLI 的 pgx 路径完全不跑 STD 校验**
（`validate` 只查 JSON Schema 形状，`commit` 只做表头投影，
STD 规则引擎在 CLI 侧从未实现，M3 已取消）。

所以「规则是硬的，agent 绕不过去」这句话**在迁移完成之前并不成立** ——
agent 走 CLI 直连库，STD 规则一条都不过。迁完之后 `commit` 必经平台的
`StandardsValidator`，ERROR 一律拦下，这句话才为真。

### `ATP_BACKEND` 一起删掉

它是迁移脚手架，删除条件当初就写明了：pgx 实现删掉时它一起删。

**留一行能切回直连数据库的配置，本身就是那条被绕开的凭证边界。**
它不会被审计发现（配置是"正常"的），只会在某次排查问题时被人临时切回去
然后忘了切回来。同样的道理适用于平台侧的 `atp.auth.enabled`。

### 架构断言顺势收紧

`arch_test.go` 原来断言「SQL 只出现在 `internal/store`」。
那个包没了，断言改成**「整个 CLI 不得出现 SQL」** ——
迁移把一条需要划边界的规则变成了一条更简单的规则。
