...
## D-125 · 两条路线离被测系统的距离不同 —— 这既是优势也是弱点

平台侧在设计「让 agent 查被测系统路由与元素」的工具时，指出了一个我没注意到的不对称：

> 保守路线那边 opencode 在客户机器上，**反而更容易做到这件事**（它就在被测环境里）——
> 你们那条路线上如果将来要补这个能力，成本比我这边低。

确实如此。opencode 就跑在用户机器上、有 shell、和被测站点同网。
真要补「查路由/元素」这个能力，一条 `curl` 加一次 DOM 提取就够了，
不需要像平台侧那样起一个 Playwright 会话。

### 但同一句话反过来读，就是 D-123

**离被测系统近，也意味着离数据库近。** D-123 那次 agent 读 `.env` 拼 SQL 删草稿，
成立的前提正是「它有 shell，且和一切都在同一台机器上」。

> **近 = 能力强，也 = 边界弱。** 这是同一个事实的两面，不能只讲前一面。

激进路线（平台内的 agent）反过来：它离被测系统远（要起 Playwright），
但**它的边界是进程和权限天然划出来的** —— 它根本碰不到用户机器上的 `.env`。

### 面试里这么讲

> 「两条路线不是『一个好一个差』，是**能力与边界的取舍方向相反**。
> 保守路线的 agent 离环境近，补能力便宜、但守边界贵；
> 激进路线的 agent 跑在平台进程里，守边界便宜、但每要一项能力都得显式造一个工具。
> 我两条都做了，所以这个对比是实测出来的，不是推演的。」

**这是「两条路线」这个叙事迄今最实的一处对比** —— 之前讲的都是做法差异，这条是结构差异。
