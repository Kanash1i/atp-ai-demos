...
#### ⭐ 落地时 `step_json` 会被规整成【纯步骤数组】

编辑期 `step_json` 是**带表头的对象**（表头暂存在里面），
commit 时表头投影进 `tc_case` 的正式列，`step_json` **同一个事务里规整成纯数组**：

```sql
UPDATE tc_step SET step_json = COALESCE(step_json->'steps', step_json) WHERE case_id = $1
```

**这一步不能省。** 保守路线的主张是「落库格式与人工案例完全一致，老执行器无感知照跑」，
而老执行器读的是数组 —— 留成对象，那句主张就是假的，
**而且它崩的时候没人知道是谁写进去的**。

顺序不能动：**必须排在读表头之后**，因为规整会把表头从 `step_json` 里抹掉。
这条 SQL 自身幂等（已是数组时 `->'steps'` 返回 NULL，`COALESCE` 原样保留），
所以 commit 的重放路径不会把内容改坏。

> ⚠️ **一个曾经声称、但被这条规整拿走的好处，要老实讲。**
> 我原先说「commit 之后 `step_json` 留着提交那一刻的**完整快照**，
> 相当于白捡了 ChangeSet 方案里的『冻结快照』」。
> **规整之后这句不成立了** —— 快照只剩步骤，表头已经被抹掉、搬进了 `tc_case` 的列。
>
> 「用户到底确认了什么」现在要拼两处才答得出，
> 而且如果平台后续改了 `tc_case`，最初确认的表头就查不回来了。
>
> **这是执行器契约赢了审计便利。** 该认 —— 执行器读不了的数据，
> 留再完整的快照也没有意义。真要审计，得单开一张表存，那是另一个设计。
...
> ❌ **未实现，且已决定不做**（2026-08-31）—— 演示不依赖它。
>
> 这一节保留的是**设计**，不是成果。保留的理由：「不加外键约束」和「锁序统一 `tc_step` → `tc_case`」这两个决策**代码里是真的**，而它们的代价（没有 `ON DELETE CASCADE`、跨表路径必须同序）正是由清理任务承担的。删掉这节，那两个决策就变成没有代价承担者的空话。
>
> **被问到时按未实现答**，别顺着这节讲成做过。
