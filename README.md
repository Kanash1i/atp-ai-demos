...
```
      ...
      ┌────────────┴───────────┐    ┌───────────┴─────────────┐
      │  保守路线                │    │  激进路线                 │
      │  demo2-atp-cli（Go）     │    │  平台内的 agent 模块        │
      │                        │    │                         │
      │  opencode 生成案例       │    │  多 agent 协作            │
      │  → 老平台格式入库         │    │  自然语言 → 案例           │
      │  → 老执行器无感知照跑      │    │  → 自己调 Playwright 执行  │
      │                        │    │  → 自己录像                │
      │  难点：幂等消费、防重放     │    │  难点：生成质量、规则约束    │
      └────────────────────────┘    └─────────────────────────┘
                        两条路线共用同一个 PG 库
```

**面试主线**：同一个平台，两种介入方式。一条把 AI 挡在平台边界之外（只负责生成，落库走老格式，
执行器完全无感知）；一条让 AI 走进来（自己写、自己跑、自己出产物）。
