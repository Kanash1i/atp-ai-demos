...
### 2.0 ⭐ 生产形态的网络隔离（约束了好几个设计，先看这个）

下面 2.1 之后那张图是**开发环境**（都在一台笔记本和一台台式机上）。
真实生产的形态是**隔离的**，而这个隔离决定了几处设计只能那么做：

```
   测试人员的机器              ATP 平台              执行机资源池          被测系统
  ┌──────────────┐         ┌──────────┐         ┌──────────────┐    ┌──────────┐
  │ opencode     │  HTTPS  │          │  队列   │  执行器进程   │    │  被测的   │
  │ + atp CLI    │ ──────► │  调度     │ ──────► │  + 浏览器     │ ─► │  Web 系统 │
  └──────────────┘         │  数据库   │         └──────────────┘    └──────────┘
         ╎                 └──────────┘                 ▲                  ▲
         ╎                       │                      └──────────────────┘
         ╎ ✗ 连不到              │ ✗ 连不到                  只有这一段能通
...
**只有执行机能连被测系统。** 测试人员的机器、平台本身都连不到 ——
不然测试环境的访问来源就乱了，也没法保证「案例跑出来的结果」是可复现的。

由此推出的三条，改动前先想清楚：

1. **页面探查必须由执行机代做。** CLI 所在的机器根本够不着被测系统，
   在 CLI 里塞一个浏览器也没用。这不是「哪种方案更好」，是只有一种方案可行。
   而且探查环境与执行环境是同一台，探到的 DOM 就是执行时会看到的 DOM。
2. **执行只能由平台派发。** CLI 的 `run` 命令调的是平台的派发接口，
   跟人在前端点「执行」走同一个 `ExecutionDispatchService` ——
   CLI 一侧没有浏览器、也不知道被测系统的地址。
3. **测试人员的机器只跟 ATP 说话。** 所以 CLI 最终必须只持 ATP 的窄 token，
   而不是数据库凭证（见 `demo2-atp-cli/CONTRACT.md` §5）。

> ⚠️ 本仓库的 demo 里 `mock-shop` 跑在局域网内、谁都能访问 —— 那是**开发便利**，
> 不是生产形态。设计决策要按上面这张图来，别按 demo 的现状来。
...
### 2.4 为什么是 TEI，不是 llama.cpp / Ollama / Infinity
