...
ATP 本身是 **Java 8 + Spring 4 / Spring Boot 2.7 + MySQL 5.7** 的遗留平台。
本服务**不嵌进它**：不共享依赖树、不碰它的数据库、只通过 HTTP 被调用。

这个定位直接决定了技术栈的自由度：

| | 同仓库的 demo1（RAG 知识助手） | 本服务 |
|---|---|---|
| 定位 | 老平台**内部**的模块 | 老平台**旁边**的新服务 |
| 栈 | Spring Boot 2.7.18 + Java 8 | Spring Boot 4.1.0 + Java 17 |
| 版本从哪来 | 被平台绑死 —— 2.7.x 是最后一个支持 Java 8 的系列 | 不被绑 —— 实测 Spring Boot 4 的 baseline 正好是 JDK 17 |

两边的做法其实一样：**版本都不是"选"出来的，是被约束算出来的**。
只是 demo1 算出了「最后一个还支持 Java 8 的版本」，
而这里算出了「最新版恰好还够得着 JDK 17」（详见 [`DECISIONS.md`](DECISIONS.md) M0-D1）。
