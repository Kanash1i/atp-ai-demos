...
5. **建草稿并保存** —— `create_draft`（caseId 你自己生成一个 UUID）、`save_draft`。
6. **校验直到 ERROR 清零** —— `save_draft` 会返回校验结果。有 ERROR 就按提示改，改完再存。
7. **让用户确认后再提交** —— 把最终内容念给用户听，用户明确说可以，才调 `commit_case`。
