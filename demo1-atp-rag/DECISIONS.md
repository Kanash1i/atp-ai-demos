...
### M3 实测发现：D 类其实是两种，评估时必须分开算

跑通生成层后发现，「应拒答」这一类里混着两种性质完全不同的问题：

| | 提问 | 语料状况 | 正确行为 |
|---|---|---|---|
| **有依据的否定** | 「ATP 支持 App 自动化吗」 | FAQ Q1 明确写了不支持 | 直接答「不支持」并引用，**不该**输出拒答标记 |
| **无依据的拒答** | 「ASSERT_JSON 怎么用」 | 语料里完全没有 | 输出 `[资料不足]` |

实测两者都正确：前者答「ATP 不支持 App 自动化…只驱动桌面浏览器 [1][2]」，
后者答「[资料不足] …平台支持的断言 action 只有三个 [2]」。

**这个区分直接影响 M4 的指标定义。** 如果把「拒答率」定义成「D 类里输出拒答标记的比例」，
那么第一种问题答对了反而会被算成失败 —— 指标会奖励一个更差的系统。

正确的定义是分开算：
- 有依据的否定 → 看**是否给出了否定结论且引用了正确来源**
- 无依据的拒答 → 看**是否输出了拒答标记**

这两种在 M1 设计语料时就刻意都留了（FAQ 里明写「不支持 App」，而 `ASSERT_JSON` 全语料不出现），
当时是为了让 D 类有层次，现在看更重要的作用是**暴露了指标定义的陷阱**。
