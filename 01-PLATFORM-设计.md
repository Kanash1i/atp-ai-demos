...
```
派发 → exec_run + N 条 exec_task（status=PENDING）
     → Redis 队列
     → atp-runner 的 worker 拉取（node-01…node-08）
     → Playwright BrowserContext(recordVideoDir=…)
     → 逐条翻译 step_json 的 Action → Playwright 调用
     → 产物落盘 + 回写 exec_task/exec_step_result
     → SSE 推进度给前端
```

### 6.2 Action → Playwright 映射
