...
**为什么静态校验必然抓不到**（与 CLI 侧 D-124 互为引用）：
**合规性是关于文本的性质，静态可判定；正确性是关于系统的性质，只有真跑一遍才知道。**
...
**下一步之一：给 agent 一个能看见被测系统的工具**（2026-08-31 定，优先于加 prompt 约束）。

> **agent 编造，通常是因为它没有查询的工具，而不是因为它不老实。**
> 加约束（"不要编造 URL，不确定就问用户"）只会让它换个方式绕；加工具才是真的把路堵上。

做法是**让 agent 真的用 Playwright 打开页面看一眼**，从 DOM 提取候选定位器
（`data-testid` / `id` / `name` / role）返回给它 —— 而不是给被测系统加一个
`/__introspect/routes` 自描述接口。理由：自描述接口在生产里不存在，
演示时方便，一问「你们真实的被测系统会给你这个接口吗」就穿了；
而「打开页面看一眼」对任何 web 系统都成立，且**执行器本来就有这个能力**，是复用不是新建。

⚠️ 这个工具的错误语义必须分两类，不能都返回"查询失败"：
路由 404 是「你查错了，换个路径或问用户」，浏览器起不来 / 站点不通是「重试或如实报告」。
**都返回"查询失败"的话，agent 分不清是自己错了还是环境错了，大概率退回编造。**

**下一步之二（方案 B，用户 2026-08-30 拍板）**：agent 提交后派发执行一次、**如实报告、人决定**。
不做自动重试闭环 —— 两个理由：
